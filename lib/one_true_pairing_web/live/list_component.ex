      ...
      <div class="space-y-5 mx-auto max-w-7xl px-4 space-y-4">
        ...
        <div id={"#{@id}-items"} phx-hook="Sortable" data-list_id={@id} test-role="list" data-group={@group}>
          <div
            :for={item <- @list}
            id={"#{@id}-#{item.id}"}
            class="drag-item:focus-within:ring-0 drag-item:focus-within:ring-offset-0 drag-ghost:bg-zinc-300 drag-ghost:border-0 drag-ghost:ring-0 m-2 border-1 border-dashed border-slate-50"
            data-id={item.id}
          >
            <div class="flex drag-ghost:opacity-0 border-5">
