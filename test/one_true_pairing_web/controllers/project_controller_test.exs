defmodule OneTruePairingWeb.ProjectControllerTest do
  ...
  describe "listing all projects" do
    test "has a header", %{conn: conn} do
      ...
      expect(response, to: match_regex(~r"All Projects"))
    end
    ...
    test "shows previously created projects", %{conn: conn} do
      ...
      expect(html, to: match_regex(~r"Project A"))
      expect(html, to: match_regex(~r"Project B"))
    end
    ...
    test "has links to manage a project", %{conn: conn} do
      ...
      expect(links, to: contain("/projects/#{project.id}"))
      expect(links, to: contain("/projects/#{project.id}/edit"))
    end
    ...
    test "can navigate to create a new project", %{conn: conn} do
      ...
      expect(link_text, to: equal("New Project"))
    end
  end
  ...
  describe "creating a project" do
    test "the new page has a form", %{conn: conn} do
      ...
      expect(header, to: equal("New Project"))

      {input, _attrs, _children} = html |> HtmlQuery.find!("#project_name")
      expect(input, to: equal("input"))

      expect(response, to: match_regex(~r"Save Project"))
      expect(response, to: match_regex(~r"Back to projects"))
    end
    ...
    test "creating redirects to show when the data is valid", %{conn: conn} do
      ...
      expect(url, to: equal(~p"/projects/#{id}"))

      response = get(conn, ~p"/projects/#{id}") |> html_response(200)
      expect(response, to: match_regex(~r"Project #{id}"))
    end
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      ...
      expect(html, to: match_regex(~r"New Project"))
      expect(html, to: match_regex(~r"Oops"))
    end
  end
  ...
  describe "editing a project" do
    test "displays the project as a form", %{conn: conn} do
      ...
      expect(response, to: match_regex(~r"Edit Project #{project.id}"))

      input_value = response |> HtmlQuery.parse() |> HtmlQuery.find!("#project_name") |> HtmlQuery.attr("value")

      expect(input_value, to: equal("Edit me"))
    end
    ...
    test "redirects when data is valid", %{conn: conn} do
      ...
      url = redirected_to(conn)
      expect(url, to: equal(~p"/projects/#{project}"))

      response = get(conn, ~p"/projects/#{project}") |> html_response(200)
      expect(response, to: match_regex(~r"Cooler project name"))
    end
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      ...
      expect(response, to: match_regex(~r"Edit Project"))
    end
  end
  ...
  describe "viewing a single project" do
    test "has a link to edit", %{conn: conn} do
      ...
      expect(response, to: match_regex(~r"/projects/#{project.id}/edit"))
    end
